			...
			if (data.status == 'ok'){
				if (data.users.length == 0){
					...
					$.each(data.users, function(idx, user){
						...
						if (user.disabled == 'false' || user.disabled == false)
							$('<button />').attr({userId: user.userId}).addClass('btn btn-danger').html('Bam').click(bamButtonClick).appendTo($('<td />').appendTo(tr));
						else 
							$('<button />').attr({userId: user.userId}).addClass('btn btn-success').html('Enable').click(enableButtonClick).appendTo($('<td />').appendTo(tr));
					});
				}
...
function bamButtonClick(e){
	var $this = $(this);
	$.ajax({
		url : '/admin/user/bam.json',
		type : 'POST',
		dataType : 'json',
		data: {userId : $(this).attr('userId')},
		success : function(data){
			if (data.status == 'ok'){
				$this.removeClass('btn-danger').addClass('btn-success').html('Enable').click(enableButtonClick);
			} else
				postErrorToDiv(data);
		},
...
function enableButtonClick(e){
	var $this = $(this);
	$.ajax({
		url : '/admin/user/unbam.json',
		type : 'POST',
		dataType : 'json',
		data: {userId : $(this).attr('userId')},
		success : function(data){
			if (data.status == 'ok'){
				$this.removeClass('btn-success').addClass('btn-danger').html('Bam').click(bamButtonClick);
			} else
				postErrorToDiv(data);
		},
		error : postConnectionErrorToDiv
